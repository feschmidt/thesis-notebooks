...
def Ij(omega,params): #If there is a Lj in params, returns the current through the junction (complex).  Else, just the load current
    Vl = Vx(0,omega,params)
    Zl = Zload(omega,params)
    Il = Vl/Zl
    return Il
...
def diff_Yq(omega,params):  #Derivative of admittance seen from JJ terminals (for anharmonicity calculation)
#Horrible formula done in mathematica
    Lg = params['Lg']
    Z0p = params['Z0p']
    l = params['l']
    Z0 = params['Z0']
    ...
    result = Complex(0,1)*Cj + Complex(0,1)/(Lj*Power(omega,2)) - \
        (Complex(0,1)*Lg - (Complex(0,0.5)*Z0*\
        Power(Sech(l*(al + (Complex(0,1)*omega)/v)),2)*\
        (-2*l*Power(Z0,2) - Cg*v*Power(Z0,3) - \
        Complex(0,4)*Cs*l*omega*Power(Z0,2)*Z0p - \
        Complex(0,2)*Cg*Cs*omega*v*Power(Z0,3)*Z0p + 2*l*Power(Z0p,2) + \
        Cg*v*Z0*Power(Z0p,2) + 2*Cs*v*Z0*Power(Z0p,2) + \
        2*Power(Cs,2)*l*Power(omega,2)*Power(Z0,2)*Power(Z0p,2) + \
        Cg*Power(Cs,2)*Power(omega,2)*v*Power(Z0,3)*Power(Z0p,2) - \
        Cg*v*Z0*(-Power(Z0p,2) + \
        Power(Z0,2)*Power(Complex(0,-1) + Cs*omega*Z0p,2))*\
        Cosh(2*l*(al + (Complex(0,1)*omega)/v)) + \
        2*Cg*v*Power(Z0,2)*Z0p*(1 + Complex(0,1)*Cs*omega*Z0p)*\
        Sinh(2*l*(al + (Complex(0,1)*omega)/v))))/\
        (v*Power(Complex(0,-1)*Z0*(Complex(0,-1) + Cg*omega*Z0p + Cs*omega*Z0p) + \
        (-Z0p + Cg*omega*Power(Z0,2)*(Complex(0,-1) + Cs*omega*Z0p))*\
        Tanh(l*(al + (Complex(0,1)*omega)/v)),2)))/\
        Power(Complex(0,1)*Lg*omega + \
        1/(Complex(0,1)*Cg*omega + \
        (Z0 + Tanh(l*(al + (Complex(0,1)*omega)/v))/\
        (Complex(0,1)*Cs*omega + 1/Z0p))/\
        (Z0*(1/(Complex(0,1)*Cs*omega + 1/Z0p) + \
        Z0*Tanh(l*(al + (Complex(0,1)*omega)/v))))),2)


    return result


def Yq(omega,params,approx=True):  #Admittance seen from JJ terminals (for anharmonicity calculation)
#Horrible formula done in mathematica
    Lg = params['Lg']
    Z0p = params['Z0p']
    l = params['l']
    ...
    v = vph
    if approx:
        result = ((Lg + Lj)*omega - 1j*Z0*np.tanh(gamma*l))/(Lj*omega*(1j*Lg*omega + Z0*np.tanh(gamma*l)))

    else:
        result = Complex(0,-1)/(Lj*omega) + Complex(0,1)*Cj*omega + 1/Rsg + \
            1/(Complex(0,1)*Lg*omega + 1/\
            (Complex(0,1)*Cg*omega + (Z0 +\
            Tanh(l*(al + (Complex(0,1)*omega)/v))/\
            (Complex(0,1)*Cs*omega + 1/Z0p))/\
            (Z0*(1/(Complex(0,1)*Cs*omega + 1/Z0p) + \
            Z0*Tanh(l*(al + (Complex(0,1)*omega)/v))))))
